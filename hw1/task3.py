...
@jit(nopython=True, fastmath=True)
def main():
    n_array = np.array([10, 100, 1000, 10000])
    ...
    for i, n in enumerate(n_array):
        ...
        u = np.zeros(n)
        A = (2 + h**2)*np.eye(n) - np.eye(n, k=1) - np.eye(n, k=-1) - \
            np.eye(n, k=n-1) - np.eye(n, k=1-n)
        f = (1 + 4*np.pi**2)*np.cos(2*np.pi*x)
        u_exact = np.cos(2*np.pi*x)
